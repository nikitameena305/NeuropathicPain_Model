TITLE 'Medlock B_A rapid A-type potassium candidate'
...
NEURON {
    ...
    RANGE gkbar, ik
    RANGE ninf, ntau, hinf, htau, tadj
}
...
PROCEDURE rates(v (mV)) {
    ...
    nalpha = 0.032*trap(-v - 64, 6)
    nbeta = 0.203*exp((-v - 40)/24)
    ninf = nalpha/(nalpha + nbeta)
    ntau = 1/(nalpha + nbeta)/tadj
    halpha = 0.05/(exp((v + 86)/10) + 1)
    hbeta = 0.05/(exp((-v - 86)/10) + 1)
